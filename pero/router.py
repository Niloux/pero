...
class EventDispatcher:
    ...
    @classmethod
    async def dispatch(cls, event: dict, api: BotAPI):
        """异步分发事件"""
        event_type = event.get("event")
        handlers = cls._event_handlers.get(event_type, [])

        if not handlers:
            _log.info(f"未处理的事件类型: {event_type}")
            return None

        # 并发执行所有处理函数
        results = await asyncio.gather(*(handler(event, api) for handler in handlers))
...
class MessageAdapter:
    ...
    @classmethod
    async def handle_message(cls, event: dict, api):
        ...
        for message_type in message_types:
            ...
            if len(handlers) == 0:
                _log.info(f"未注册的消息类型: {message_type}")
                continue
...
@EventDispatcher.register_event("message")
async def handle_message_event(event: dict, api):
    """处理 message 事件"""
    await MessageAdapter.handle_message(event, api)
