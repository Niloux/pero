...
class PluginManager:
    ...
    def load_plugins(self, api: BotAPI):
        """加载插件目录中的插件"""
        for filename in os.listdir(self.plugin_dir):
            if filename.endswith(".py"):
                ...
                plugin_instance = plugin_class(api)  # 实例化插件类
    ...
    async def handle_private_message(self, msg: PrivateMessage):
        ...
        _log.debug(f"Handling private message: {msg}")
        for plugin in self.plugins:
            if hasattr(plugin, "on_private_message"):
                _log.debug(f"Calling plugin {plugin} on_private_message")
